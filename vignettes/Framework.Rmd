...
```{r}
map10km$prob <- createOccProb(map10km, 
                              intercept = 0.5,
                              sigmaFylke = 0.4, 
                              sigmaKommune = 0.1, 
                              sigmaGrid = 0.05, 
...
```
```{r}
sample5years <- sampleBin(years5, column = "prob", nSites = 400, nObs = 10)
```

```{r}
require(lme4)
mod4 <- glmer(cbind(nFound, nVisits - nFound) ~ year + (1|fylke) + (1|kommune), family = "binomial", data = sample5years)
